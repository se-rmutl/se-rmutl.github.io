...
## Revision note
- Updated `images/module-architecture.svg` (v3): redesigned as a hub-and-spoke map with `main.js` at the center. Every request/return line uses its own empty lane, so connectors never cross module cards, labels, or the entry-point code.
